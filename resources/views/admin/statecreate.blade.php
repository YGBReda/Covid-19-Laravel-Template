...
<form class="uk-form-stacked add-zone" action="{{ url('state/store') }}" method="post" enctype="multipart/form-data">
{{ csrf_field() }}
    ...
    <div class="uk-margin uk-form-width-large">
        ...
        <div class="uk-form-controls">
            <select class="uk-select" id="State" name="state">
                @foreach($stateshare as  $stateshare)
                  <option>{{$stateshare->name}}</option>
                  @endforeach
